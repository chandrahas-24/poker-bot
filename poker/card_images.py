...
# Card Skins — "overlay" variant, corner-pip preservation. When a skin sets
# `preserve_corners: True` (see db.SKINS), the card's own top-left and
# bottom-right corners (where the rank + suit index sits) are re-stamped on
# top of the overlay art after compositing, so the overlay can never cover
# them — regardless of whether the overlay png itself has any transparency
# there. These fractions are generic estimates of how much of a standard
# card's corner the rank+suit index occupies; tune here if a given overlay
# still clips them.
SKIN_CORNER_WIDTH_RATIO  = 0.30
SKIN_CORNER_HEIGHT_RATIO = 0.30
...
# Card Skins cosmetic — never applied to shiny cards.
# "overlay"/"mask_underlay" variants: raw (un-resized) art, keyed by filename —
# resized on demand against whatever card it's being applied to.
_skin_asset_cache: dict[str, Image.Image] = {}
# "full_replace" variant: skin_id -> {card_int: resized art}, same shape as _card_cache.
_skin_full_deck_cache: dict[str, dict[int, Image.Image]] = {}
_skin_full_deck_cache_compact: dict[str, dict[int, Image.Image]] = {}
...
def _load_cache():
    ...
    # Card Skins
    for skin_id, info in db.SKINS.items():
        if info.get("variant") == "full_replace":
            deck_dir = info.get("deck_dir")
            if not deck_dir:
                print(f"⚠️  Card Skin {skin_id} is full_replace but has no deck_dir configured")
                continue
            deck_base = os.path.join(CARDS_DIR, deck_dir)
            deck_cache: dict[int, Image.Image] = {}
            deck_cache_compact: dict[int, Image.Image] = {}
            for rank in RANK_NAMES.keys():
                for suit in SUIT_NAMES.keys():
                    card_int = Card.new(f"{rank}{suit}")
                    fpath = os.path.join(deck_base, card_filename(card_int))
                    if os.path.exists(fpath):
                        deck_cache[card_int] = _resize(fpath, CARD_HEIGHT)
                        deck_cache_compact[card_int] = _resize(fpath, CARD_HEIGHT_COMPACT)
            if deck_cache:
                _skin_full_deck_cache[skin_id] = deck_cache
                _skin_full_deck_cache_compact[skin_id] = deck_cache_compact
                print(f"🎴 {info.get('display', skin_id)} full-deck skin cached ({len(deck_cache)} cards)")
            else:
                print(f"⚠️  Card Skin {skin_id} full_replace deck missing: expected {deck_base}")
        else:
            # overlay / mask_underlay — eagerly warm the asset cache so the
            # first render doesn't take a disk hit.
            for field in ("overlay", "overlay_red", "overlay_black", "easter_egg_overlay", "underlay"):
                fname = info.get(field)
                if fname:
                    _load_skin_asset(fname)
...
def _load_skin_asset(filename: str) -> Image.Image | None:
    """Lazily load + cache a Card Skin's raw art (overlay/underlay png) by filename."""
    if filename in _skin_asset_cache:
        return _skin_asset_cache[filename]
    path = os.path.join(CARDS_DIR, filename)
    if os.path.exists(path):
        img = Image.open(path).convert("RGBA")
        _skin_asset_cache[filename] = img
        return img
    print(f"⚠️  Card Skin asset missing: expected {path}")
    return None


def _apply_mask_underlay(card_img: Image.Image, underlay: Image.Image, white_threshold: int = 235) -> Image.Image:
    """
    Card Skins — "mask_underlay" variant.
    Cuts the (near-)white background out of the card art (keeping its ink —
    rank, suit, border lines) and shows `underlay` through the gap.
    """
    card = card_img.convert("RGBA")
    r, g, b, a = card.split()
    min_rgb = ImageChops.darker(ImageChops.darker(r, g), b)
    # 255 = keep the card's own pixel (it's not white), 0 = let the underlay show through
    keep_mask = min_rgb.point(lambda v: 255 if v < white_threshold else 0)
    new_alpha = ImageChops.multiply(a, keep_mask)
    masked_card = Image.merge("RGBA", (r, g, b, new_alpha))

    base = underlay.resize(card.size, Image.LANCZOS).convert("RGBA").copy()
    base.alpha_composite(masked_card)
    return base


def _preserve_corner_pips(original: Image.Image, composited: Image.Image) -> Image.Image:
    """
    Re-stamps `original`'s top-left and bottom-right corners (where a
    standard card's rank + suit index sits) back on top of `composited`,
    so a skin overlay can never obscure them — independent of whatever
    transparency the overlay art itself has there.
    """
    w, h = original.size
    cw = max(1, round(w * SKIN_CORNER_WIDTH_RATIO))
    ch = max(1, round(h * SKIN_CORNER_HEIGHT_RATIO))
    result = composited.convert("RGBA").copy()
    tl = original.crop((0, 0, cw, ch))
    result.paste(tl, (0, 0), tl)
    br = original.crop((w - cw, h - ch, w, h))
    result.paste(br, (w - cw, h - ch), br)
    return result


def _apply_card_skin(img: Image.Image, card_int: int, skin_id: str, batman: bool = False) -> Image.Image:
    """
    Card Skins cosmetic — dispatches on the skin's `variant`. Never called for
    shiny cards or the card back (callers are expected to check that first).
    `full_replace` skins have already had their base art swapped in by the
    caller's card lookup, so there's nothing left to do here for them.
    """
    ...
    if variant == "overlay":
        overlay_name = None
        if batman and info.get("easter_egg_overlay"):
            overlay_name = info["easter_egg_overlay"]
        elif info.get("overlay"):
            overlay_name = info["overlay"]
        elif info.get("overlay_red") or info.get("overlay_black"):
            suit = Card.int_to_str(card_int)[1]
            overlay_name = info["overlay_red"] if suit in ("h", "d") else info["overlay_black"]
        overlay = _load_skin_asset(overlay_name) if overlay_name else None
        if overlay is None:
            return img
        base = img.convert("RGBA")
        resized = overlay.resize(base.size, Image.LANCZOS)
        out = Image.alpha_composite(base, resized)
        if info.get("preserve_corners"):
            out = _preserve_corner_pips(base, out)
        return out

    if variant == "mask_underlay":
        underlay = _load_skin_asset(info["underlay"]) if info.get("underlay") else None
        if underlay is None:
            return img
        return _apply_mask_underlay(img, underlay)

    # "full_replace" (or unknown) — nothing more to apply here.
    return img
...
def make_strip(card_ints: list[int], backs: int = 0, is_hole: bool = False, shiny_ids: list[str] | None = None,
                ...
                cute_mode: bool = False, border_id: str | None = None, compact: bool = False,
                skin_id: str | None = None, skin_batman_card: int | None = None) -> discord.File:
    """
    ...
    `skin_id` is the player's active Card Skin (see db.SKINS) — never applied
    to shiny cards. `skin_batman_card` is the specific card_int (if any) that
    should render with the skin's rare "easter egg" art instead of its normal
    art this hand (chosen once per hand by the caller, so it stays put across
    repeated renders — see poker.py).
    """
    ...
    skin_deck_cache = (_skin_full_deck_cache_compact if compact else _skin_full_deck_cache).get(skin_id, {})
    ...
    # Cute Mode (chaos modifier) and the "cute" Card Skin use the same overlay
    # art — if both are in play at once, apply it once, not twice.
    effective_skin_id = None if (skin_id == "cute" and cute_mode) else skin_id

    # Use cached images if available, fall back to live resize if cache missed
    def _get_card(card_int: int) -> tuple[Image.Image, bool]:
        """Returns (image, is_shiny)."""
        for sid in shiny_ids:
            shiny = shiny_cards.get_by_id(sid)
            if shiny and shiny.card_int == card_int and sid in shiny_cache:
                return shiny_cache[sid].copy(), True
        # Card Skins: full deck replacement swaps the base art (never for shiny cards)
        if card_int in skin_deck_cache:
            return skin_deck_cache[card_int].copy(), False
        if card_int in card_cache:
            return card_cache[card_int].copy(), False
        return _resize(card_path(card_int), target_height), False
    ...
    for idx, c in enumerate(card_ints):
        if c == BACK_SENTINEL:
            ...
            continue
        img, is_shiny = _get_card(c)
        if effective_skin_id and not is_shiny:
            img = _apply_card_skin(img, c, effective_skin_id, batman=(skin_batman_card == c))
